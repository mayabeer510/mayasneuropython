...
# ------------------------------------------------------------------
# Question 5: Correlation Between Gender, Age Group and Responses
# ------------------------------------------------------------------

    def correlate_gender_age(self) -> pd.DataFrame:
        df = self.data.copy()

        # Select only question-related columns
        q_cols = [col_name for col_name in df.columns if col_name.startswith("q")]

        # Ensure numeric types for processing
        df[q_cols] = df[q_cols].apply(pd.to_numeric, errors="coerce")
        df["age"] = pd.to_numeric(df["age"], errors="coerce")

        # Filter out incomplete rows for gender and age
        valid_rows = df["gender"].notnull() & df["age"].notnull()
        df = df.loc[valid_rows]

        # Convert age to binary group
        df["age"] = df["age"] > 40

        # Impute missing values per group mean
        df[q_cols] = (
            df.groupby(["gender", "age"])[q_cols]
